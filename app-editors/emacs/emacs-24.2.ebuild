...
SRC_URI="mirror://gnu/emacs/${P}.tar.xz
	...
	http://umiushi.org/~wac/bgex/patch-bgex_20120916_0-emacs-24.2.tar.bz2"
...
IUSE="alsa aqua athena dbus games gconf gif gnutls gpm gsettings gtk gtk3 gzip-el hesiod imagemagick jpeg kerberos libxml2 livecd m17n-lib motif pax_kernel png selinux sound source svg tiff toolkit-scroll-bars wide-int X Xaw3d xft +xpm bgex"
REQUIRED_USE="aqua? ( !X )"
...
src_prepare() {
	...
	if use bgex; then
		# Background Extension Patch
		epatch ${WORKDIR}/patch-bgex_20111215_0-bzr-emacs-trunk/patch-bgex_20111215_0-bzr-emacs-trunk
	fi
...
src_compile() {
	...
	if use bgex; then
		elisp-compile "${WORKDIR}/patch-bgex_20111215_0-bzr-emacs-trunk/bgex.el"
	fi
}
...
src_install () {
	...
	if use bgex; then
		cp ${WORKDIR}/patch-bgex_20111215_0-bzr-emacs-trunk/bgex.el* ${ED}/usr/share/emacs/${FULL_VERSION}/lisp/
	fi
}
